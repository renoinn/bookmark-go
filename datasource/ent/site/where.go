...
// HasBookmarkFrom applies the HasEdge predicate on the "bookmark_from" edge.
func HasBookmarkFrom() predicate.Site {
	return predicate.Site(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(BookmarkFromTable, FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, BookmarkFromTable, BookmarkFromColumn),
		)
		sqlgraph.HasNeighbors(s, step)
	})
}

// HasBookmarkFromWith applies the HasEdge predicate on the "bookmark_from" edge with a given conditions (other predicates).
func HasBookmarkFromWith(preds ...predicate.Bookmark) predicate.Site {
	return predicate.Site(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(BookmarkFromInverseTable, FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, BookmarkFromTable, BookmarkFromColumn),
		)
