...
CREATE TABLE `bookmarks` (`id` bigint NOT NULL AUTO_INCREMENT, `title` varchar(255) NOT NULL, `note` varchar(1000) NOT NULL, `site_id` bigint NOT NULL, `user_id` bigint NOT NULL, PRIMARY KEY (`id`), CONSTRAINT `bookmarks_sites_bookmark_from` FOREIGN KEY (`site_id`) REFERENCES `sites` (`id`) ON DELETE NO ACTION, CONSTRAINT `bookmarks_users_bookmarks` FOREIGN KEY (`user_id`) REFERENCES `users` (`id`) ON DELETE NO ACTION) CHARSET utf8mb4 COLLATE utf8mb4_bin;
-- create "tags" table
CREATE TABLE `tags` (`id` bigint NOT NULL AUTO_INCREMENT, `user_id` bigint NOT NULL, `name` varchar(255) NOT NULL, `count` bigint NOT NULL DEFAULT 0, `user_tags` bigint NOT NULL, PRIMARY KEY (`id`), CONSTRAINT `tags_users_tags` FOREIGN KEY (`user_tags`) REFERENCES `users` (`id`) ON DELETE NO ACTION) CHARSET utf8mb4 COLLATE utf8mb4_bin;
