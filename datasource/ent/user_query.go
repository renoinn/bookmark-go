...
func (uq *UserQuery) loadBookmark(ctx context.Context, query *BookmarkQuery, nodes []*User, init func(*User), assign func(*User, *Bookmark)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(user.BookmarkTable)
		s.Join(joinT).On(s.C(bookmark.FieldID), joinT.C(user.BookmarkPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(user.BookmarkPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(user.BookmarkPrimaryKey[0]))
...
func (uq *UserQuery) loadTag(ctx context.Context, query *TagQuery, nodes []*User, init func(*User), assign func(*User, *Tag)) error {
	...
	query.Where(func(s *sql.Selector) {
		...
		s.Join(joinT).On(s.C(tag.FieldID), joinT.C(user.TagPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(user.TagPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(user.TagPrimaryKey[1]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
